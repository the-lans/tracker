...
# ../alembic/env.py и ../scripts/ — обычный код вне backend/, проверяется
# вместе с ним. Остальное в alembic/ (versions/*.py) — сгенерированные Alembic
# миграции, их не линтим: шаблон script.py.mako сам импортирует op/sa и Union,
# даже когда конкретная миграция их не использует (как эта, пустая) — ruff
# закономерно ругался бы на unused-import в каждой второй миграции.
backend-lint: $(BACKEND_VENV_MARKER)
	cd $(BACKEND_DIR) && .venv/bin/ruff check . ../alembic/env.py ../scripts

backend-typecheck: $(BACKEND_VENV_MARKER)
	cd $(BACKEND_DIR) && .venv/bin/mypy . ../alembic/env.py ../scripts
